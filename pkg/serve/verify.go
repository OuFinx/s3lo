...
// Verifier loads a public or KMS key at startup and checks manifest signatures on
// request, so an image without a valid signature never reaches a client.
...
// IsVerified reports whether a manifest with the given digest has already passed
// signature verification in this process. Signatures are keyed by tag, so a bare
// digest pull can only be trusted if the same content was verified via its tag.
func (v *Verifier) IsVerified(digest string) bool {
	_, ok := v.cache.Load(digest)
	return ok
}
