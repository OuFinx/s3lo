...
Yes. Just push to a `local://` reference (e.g. `s3lo push myapp:v1.0 local://./my-store/myapp:v1.0`) — the OCI layout is created on demand, no setup step. All commands — push, pull, list, inspect, delete, copy — work with local storage.
