...
s3lo does not scan images itself — dedicated scanners do it better and are
already in most pipelines. Pull the image and hand it to one:

```yaml
      - name: Scan
        run: |
          ./s3lo pull s3://my-bucket/myapp:${{ github.sha }} --platform linux/amd64
