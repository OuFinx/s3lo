...
- name: Verify image signature
  run: |
    s3lo verify s3://my-bucket/myapp:${{ github.sha }} \
      --key awskms:///alias/release-signer \
      --output json > verify-result.json
  env:
...
!!! warning "Do not pipe the result"
    Write to a file with `>`, not `| tee verify-result.json`. A pipeline's exit
    status is the *last* command's, so `tee` returning 0 masks verify's exit 1 and
    the step passes on an invalid signature. If you must pipe, set
    `shell: bash` on the step and start the script with `set -o pipefail`.
